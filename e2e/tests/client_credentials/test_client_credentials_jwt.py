...
from e2e.scripts.config import OAUTH_URL


@pytest.mark.asyncio
class TestClientCredentialsJwtUnattendedAccess:
    ...
    async def _check_client_credentials_token_response(self, jwt_claims, helper, expected_response,
    ...
    async def test_incorrect_jwt_algorithm(self, helper):
        ...
        await self._check_client_credentials_token_response(jwt_claims, helper, expected_response, expected_status_code)
    ...
    async def test_successful_jwt_token_response(self):
        ...
        assert resp['body'].get(
            'expires_in') == '599', f"UNEXPECTED 'expires_in' {resp.get('expires_in')} {resp['body']}"
