...
class TestSplunkLogging:
    ...
    async def test_access_token_fields_for_logging_when_using_authorization_code_cis2(
        ...
        requests.get(
            f"{HELLO_WORLD_API_URL}",
            headers={"Authorization": f"Bearer {self.oauth.access_token}"},
        )

        # Then
        auth_type = await apigee_trace.get_apigee_variable_from_trace(
            name="accesstoken.auth_type"
        )
        auth_grant_type = await apigee_trace.get_apigee_variable_from_trace(
            name="accesstoken.auth_grant_type"
        )
        auth_level = await apigee_trace.get_apigee_variable_from_trace(
            name="accesstoken.auth_level"
        )
        auth_provider = await apigee_trace.get_apigee_variable_from_trace(
            name="accesstoken.auth_provider"
        )
    ...
    async def test_access_token_fields_for_logging_when_using_authorization_code_nhs_login(
        self, scope, helper
    ):
        # Given
        apigee_trace = ApigeeApiTraceDebug(proxy=f"hello-world-{ENVIRONMENT}")
        ...
        response = await self.oauth.hit_oauth_endpoint(
            ...
            data={"state": state, "auth_method": scope},
            allow_redirects=False,
        )
    ...
    @pytest.mark.logging
    @pytest.mark.parametrize("scope", ["P9", "P5", "P0"])
    async def test_access_token_fields_for_logging_when_using_token_exchange_nhs_login(
        self, scope
    ):
        # Given
        apigee_trace = ApigeeApiTraceDebug(proxy=f"hello-world-{ENVIRONMENT}")
