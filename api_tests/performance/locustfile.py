import os
import json
import urllib.parse as urlparse
from urllib.parse import parse_qs
from locust import SequentialTaskSet, HttpUser, task, constant, tag


class IdentityServiceUser(HttpUser):
    wait_time = constant(1)

    @task
    class AuthenticationSequence(SequentialTaskSet):

        def on_start(self):
            """ Environment Variables """
            self.base_url = os.environ["LOCUST_HOST"]
            self.identity_proxy = self._identity_proxy_name()
            self.client_id = os.environ["CLIENT_ID"]
            self.client_secret = os.environ["CLIENT_SECRET"]
            self.callback_url = os.environ["CALLBACK_URL"]
            """ Flow Data """
            self.state = None
            self.redirect_uri = None
            self.auth_code = None
            self.credentials = None

        def _identity_proxy_name(self):
            try:
                namespace = os.environ["NAMESPACE"]
                return f"oauth2-{namespace}"
            except KeyError:
                return "oauth2"

        @task
        @tag('authorize', 'callback', 'code', 'token', 'refresh')
        def _get_state(self):
            params = {
                "client_id": self.client_id,
                "redirect_uri": self.callback_url,
                "response_type": "code",
                "state": "1234567890"
            }
            with self.client.get(f"/{self.identity_proxy}/authorize", params=params, catch_response=True) as response:
                parsed = urlparse.urlparse(response.url)
                self.state = parse_qs(parsed.query)['state'][0]

        @task
        @tag('callback', 'code', 'token', 'refresh')
        def _get_redirect_callback(self):
            url = f"/{self.identity_proxy}/simulated_auth"
            params = {
                "response_type": "code",
                "client_id": "some-client-id",
                "redirect_uri": f"{self.base_url}/callback",
                "scope": "openid",
                "state": self.state
            }
            headers = {
                "Accept": "*/*",
                "Accept-Encoding": "gzip,deflate",
                "Cache-Control": "no-cache",
                "Content-Type": "application/x-www-form-urlencoded"
            }
            payload = {
                "state": self.state
            }
            with self.client.post(url, params=params, data=payload, headers=headers, allow_redirects=False) as response:
                redirect_uri = response.headers['Location']
                redirect_uri = redirect_uri.replace("oauth2", self.identity_proxy)
                self.redirect_uri = redirect_uri

        @task
        @tag('code', 'token', 'refresh')
        def _get_auth_code(self):
            with self.client.get(self.redirect_uri, allow_redirects=False) as response:
                parsed = urlparse.urlparse(response.headers['Location'])
                self.auth_code = parse_qs(parsed.query)["code"][0]

        @task
        @tag('token', 'refresh')
        def _get_access_token(self):
            url = f"/{self.identity_proxy}/token"
            headers = {
                "Accept": "*/*",
                "connection": "keep-alive",
                "Content-Type": "application/x-www-form-urlencoded",
            }
            payload = {
                "grant_type": "authorization_code",
                "code": self.auth_code,
                "redirect_uri": self.callback_url,
                "client_id": self.client_id,
                "client_secret": self.client_secret
            }
            with self.client.post(url, data=payload, headers=headers) as response:
                credentials = json.loads(response.text)
                self.credentials = credentials

        @task
        @tag('refresh')
        def _refresh_token(self):
            headers = {
                "Content-Type": "application/x-www-form-urlencoded"
            }
            payload = {
                "grant_type": "refresh_token",
                "client_id": self.client_id,
                "client_secret": self.client_secret,
                "refresh_token": self.credentials["refresh_token"]
            }
            self.client.post(f"/{self.identity_proxy}/token", headers=headers, data=payload)
